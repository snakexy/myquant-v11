...
### 8. 复权因子预计算和混合模式缓存（已完成）
**问题**: 切换周期时重复计算复权因子（50-200ms），XDXR数据虽缓存但复权计算无缓存
**解决**: 实现 `AdjustmentFactorService`，支持混合模式复权

**混合模式策略**:
- **日线(1d)**: 使用传统前复权（front）- 累积因子，最新价不变
- **分钟线(1m/5m/15m/30m/1h)**: 使用等比前复权（front_ratio）- 独立因子，不依赖完整历史

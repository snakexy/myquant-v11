...
export function defaultTickMarkFormatter(timePoint: TimePoint, tickMarkType: TickMarkType, locale: string): string {
	...
	switch (tickMarkType) {
		...
		case TickMarkType.Time:
			...
			return `${String(hours).padStart(2, '0')}:${String(date.getUTCMinutes()).padStart(2, '0')}`;
		case TickMarkType.TimeWithSeconds:
			// UTC 时间 +8 小时 = 北京时间
			const hoursSec = (date.getUTCHours() + 8) % 24;
			return `${String(hoursSec).padStart(2, '0')}:${String(date.getUTCMinutes()).padStart(2, '0')}:${String(date.getUTCSeconds()).padStart(2, '0')}`;
		default:
			ensureNever(tickMarkType);
			return '';
	}

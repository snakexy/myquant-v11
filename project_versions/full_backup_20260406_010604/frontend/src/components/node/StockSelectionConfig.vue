<template>
  <div class="stock-selection-config">
      ...
      <div class="time-range-group">
        <div class="time-presets">
          <button
            ...
            :disabled="isTimeRangeDisabled(preset)"
            :title="isTimeRangeDisabled(preset) ? '分钟级数据最多只能选择1个月时间范围' : preset.label"
          >
            {{ preset.label }}
          </button>
        </div>
    ...
    <div class="config-section">
      ...
      <div class="frequency-description" v-if="isIntradayFrequency">
        <small class="notice-text">
          ⚠️ 分钟级数据最多只能选择1个月时间范围
        </small>
      </div>
...
const dataFrequencies = [
  { label: '1分钟', value: '1min', description: '超短线，最多1个月数据', intraday: true },
  { label: '5分钟', value: '5min', description: '短线交易分析，最多1个月数据', intraday: true },
  { label: '15分钟', value: '15min', description: '适合日内交易，最多1个月数据', intraday: true },
  { label: '30分钟', value: '30min', description: '适合波段操作，最多1个月数据', intraday: true },
  { label: '60分钟', value: '60min', description: '适合中短期趋势，最多1个月数据', intraday: true },
...
const isIntradayFrequency = computed(() => {
  ...
  return freq === '1min' || freq === '5min' || freq === '15min' ||
         freq === '30min' || freq === '60min' || freq === '1min'
})

// 计算时间范围选项是否应该被禁用
// 分钟级数据（15分钟及以下）最多只能选1个月
...
watch(() => config.frequency, (newFreq, oldFreq) => {
  ...
  // 检查是否切换到分钟级频率
  const wasIntraday = oldFreq === '1min' || oldFreq === '5min' || oldFreq === '15min' ||
                     oldFreq === '30min' || oldFreq === '60min'
  const isIntraday = newFreq === '1min' || newFreq === '5min' || newFreq === '15min' ||
                    newFreq === '30min' || newFreq === '60min'

  if (isIntraday && !wasIntraday) {
    // 切换到分钟级数据，检查当前时间范围
    if (config.timeRange !== '1W' && config.timeRange !== '1M') {
      // 如果当前时间范围超过1个月，自动切换到1个月
      config.timeRange = '1M'
      // 更新日期范围
      ...
      console.log(`[频率切换] 分钟级数据自动调整为1个月: ${config.startDate} 到 ${config.endDate}`)
    }
  }
})
...
const selectTimeRange = (preset: { label: string; value: string; days: number }) => {
  // 检查是否是分钟级频率且时间范围超过1个月
  if (isIntradayFrequency.value && preset.days > 30) {
    console.warn('[时间范围选择] 分钟级数据不能选择超过1个月的时间范围')
    return // 不执行切换
  }

          ...
          <div class="trade-section">
            ...
            <div class="info-grid">
              ...
              <div class="info-item">
                ...
                <div class="info-value">{{ currentQuote.volume_ratio || '--' }}</div>
              </div>
              <div class="info-item">
              ...
              <div class="info-item">
                <div class="info-label">{{ isZh ? '历史最高' : '52W High' }}</div>
                <div class="info-value">{{ currentQuote.his_high || '--' }}</div>
              </div>
              <div class="info-item">
                <div class="info-label">{{ isZh ? '历史最低' : '52W Low' }}</div>
                <div class="info-value">{{ currentQuote.his_low || '--' }}</div>
              </div>
...
const updateQuoteFromSnapshot = (quote: any) => {
  ...
  currentQuote.value = {
    ...
    his_high: quote.his_high || quote.HisHigh || 0,
    his_low: quote.his_low || quote.HisLow || 0,
...
const updateOrderBookFromSnapshot = (quote: QuoteSnapshot) => {
  ...
  asks.value = [
    { price: quote.ask5 != null ? Number(quote.ask5).toFixed(2) : '--', size: quote.ask5_vol ?? 0 },
    { price: quote.ask4 != null ? Number(quote.ask4).toFixed(2) : '--', size: quote.ask4_vol ?? 0 },
    { price: quote.ask3 != null ? Number(quote.ask3).toFixed(2) : '--', size: quote.ask3_vol ?? 0 },
    { price: quote.ask2 != null ? Number(quote.ask2).toFixed(2) : '--', size: quote.ask2_vol ?? 0 },
    { price: quote.ask1 != null ? Number(quote.ask1).toFixed(2) : '--', size: quote.ask1_vol ?? 0 }
  ]

  // 买盘（价格从低到高：bid1 → bid5）
  bids.value = [
    { price: quote.bid1 != null ? Number(quote.bid1).toFixed(2) : '--', size: quote.bid1_vol ?? 0 },
    { price: quote.bid2 != null ? Number(quote.bid2).toFixed(2) : '--', size: quote.bid2_vol ?? 0 },
    { price: quote.bid3 != null ? Number(quote.bid3).toFixed(2) : '--', size: quote.bid3_vol ?? 0 },
    { price: quote.bid4 != null ? Number(quote.bid4).toFixed(2) : '--', size: quote.bid4_vol ?? 0 },
    { price: quote.bid5 != null ? Number(quote.bid5).toFixed(2) : '--', size: quote.bid5_vol ?? 0 }
  ]
}

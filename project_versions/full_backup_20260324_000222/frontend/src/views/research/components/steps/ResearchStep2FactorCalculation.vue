...
const runCalculation = async () => {
  ...
  try {
    ...
    console.log('使用日期范围:', dates)
    console.log('股票池:', props.stockPool)
    ...
    console.log('Date range:', dates.startDate, '-', dates.endDate)

    // 尝试调用真正的因子计算 API
    try {
      const axios = (await import('axios')).default
      const response = await axios.post('/api/v1/research/factor/calculate', {
        factor_type: selectedCalcTypes.value[0] || 'alpha158',
        symbols: [],  // 使用默认股票池
      ...
      if (response.data?.success) {
        // API 调用成功，使用返回的任务 ID
        newTask.task_id = response.data.data.task_id
        console.log('因子计算任务已提交:', response.data.data)
      }
    } catch (apiError: any) {
      console.warn('API 调用失败，使用模拟计算:', apiError.message)
    }

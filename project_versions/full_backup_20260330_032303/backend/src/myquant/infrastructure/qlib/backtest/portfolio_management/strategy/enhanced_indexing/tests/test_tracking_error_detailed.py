...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), 'risk_management'))
...
class TestTrackingErrorController(unittest.TestCase):
    ...
    def test_tracking_error_contribution(self):
        ...
        self.assertEqual(len(contributions), self.n_factors + 1)  # +1 for idiosyncratic
    ...
    def test_tracking_error_history_update(self):
        ...
        # 验证历史记录
        self.assertEqual(len(self.controller.tracking_error_history), len(errors))
    ...
    def test_tracking_error_statistics(self):
        ...
        # 验证超限次数
        exceeds_count = sum(1 for e in errors if e > self.controller.max_tracking_error)
    ...
    def test_real_time_monitoring(self):
        ...
        for i in range(monitoring_period):
            ...
            monitor_controller.update_tracking_error_history(current_error, timestamp)
    ...
    def test_multiple_optimization_algorithms(self):
        ...
        # 验证不同策略的行为
        self.assertFalse(results['保守策略']['should_rebalance'])  # 0.03 > 0.02 但 < 0.02+0.005
        self.assertTrue(results['激进策略']['should_rebalance'])   # 0.03 < 0.08 但变化可能超过阈值
    ...
    def test_performance_optimization(self):
        ...
        large_factor_exposure = np.random.randn(large_n_assets, large_n_factors)
        large_factor_covariance = np.random.randn(large_n_factors, large_n_factors)
        large_factor_covariance = large_factor_covariance @ large_factor_covariance.T

  ...
  <div class="task-config-info">
    ...
    <div class="config-item">
      <span class="config-label">
        ...
        {{ isZh ? '数据操作' : 'Data Operations' }}
      </span>
      <div class="data-ops-buttons">
        <el-button
          size="small"
          type="primary"
          :loading="loadingData"
        ...
        >
          <svg v-if="!loadingData" class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
            <polyline points="7 10 12 15 17 10"/>
            <line x1="12" y1="15" x2="12" y2="3"/>
          </svg>
          {{ isZh ? '获取数据' : 'Fetch Data' }}
        </el-button>
        <el-button
          size="small"
        ...
        >
          <svg v-if="!updatingData" class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="23 4 23 10 17 10"/>
            <polyline points="1 20 1 14 7 14"/>
            <path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/>
          </svg>
          {{ isZh ? '更新数据' : 'Update Data' }}
        </el-button>
        <el-button
          size="small"
          @click="handleCheckDataStatus"
        >
          <svg class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="20 6 9 17 4 12"/>
          </svg>
          {{ isZh ? '检查状态' : 'Check Status' }}
        </el-button>
      </div>
    </div>
...
const handleFetchData = async () => {
  ...
  try {
    ...
    const instruments = poolMap[props.stockPool] || props.stockPool

    const response = await axios.post('/api/v1/research/data/refresh', {
      force: true,
      symbols: props.stockPool === 'custom' ? props.customStocks?.split(',') : undefined
    })

    if (response.data?.code === 200) {
      // 刷新后检查状态
      await handleCheckDataStatus()
    }
  } catch (error: any) {
...
const handleCheckDataStatus = async () => {
  try {
    ...
    if (response.data?.code === 200) {
      ...
      dataStatus.value = {
        hasData: data.l3_stocks > 0,
        stockCount: data.l3_stocks || 0,
        dateRange: `${data.l3_last_update || '-'}`,
        lastUpdate: data.l3_last_update || '-'
      }
    }
  } catch (error: any) {

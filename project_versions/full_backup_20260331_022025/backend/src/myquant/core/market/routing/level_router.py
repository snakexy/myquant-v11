...
class LevelRouter:
    ...
    LEVEL_CONFIGS: Dict[DataLevel, LevelConfig] = {
        ...
        # stock: xtquant 优先（快）→pytdx 回退
        # index: pytdx 优先（覆盖更全）→xtquant 回退
        # sector: PyTdx(24/7) →TdxQuant
        DataLevel.L3: LevelConfig(
            stock_sources=['localdb', 'xtquant', 'pytdx', 'tdxquant'],
            index_sources=['localdb', 'pytdx', 'xtquant', 'tdxquant'],
            sector_sources=['pytdx', 'tdxquant'],
        ),

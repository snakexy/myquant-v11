...
export function transformUnifiedKlineData(
  ...
  return apiData.map(item => {
    let time: number | { year: number; month: number; day: number }

    if (typeof item.time === 'number') {
      // 后端返回的是毫秒时间戳
      const timestampMs = item.time
      const timestampSec = Math.floor(timestampMs / 1000)
      // 【关键】前端加 8 小时偏移，把数据点"搬"到 UTC 15:00 的位置
      time = timestampSec + 8 * 3600
      console.log(`[时间转换] 后端=${timestampMs} (${timestampSec}s) -> +8h=${time}`)
      ...
      if (timePart) {
        ...
        time = Math.floor(localDate.getTime() / 1000) + 8 * 3600
      } else {
        time = { year, month, day }
      }
    } else {
      const now = new Date()
      time = Date.UTC(now.getFullYear(), now.getMonth(), now.getDate()) / 1000
    }

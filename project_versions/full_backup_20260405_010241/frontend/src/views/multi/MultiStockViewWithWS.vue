  ...
  <div class="multi-stock-view">
    ...
    <div class="control-bar">
      ...
      <div class="status-bar">
        ...
        <div class="refresh-info" v-if="lastUpdateTime">
          最后更新: {{ lastUpdateTime }}
        </div>
      </div>
      <div class="stock-selector">
        ...
        <el-button @click="fetchQuotes" :loading="refreshing">
    ...
    <div class="stock-grid" :class="`grid-${layoutMode}`">
      <div
        ...
        :key="stock.symbol"
        class="stock-card"
        :class="{ updated: stock.justUpdated }"
      ...
      >
        ...
        <div class="update-indicator" v-if="stock.justUpdated">
...
import { Plus, Search, Refresh, Connection, RefreshRight } from '@element-plus/icons-vue'
...
const fetchQuotes = async () => {
  try {
    ...
    // 更新行情数据
    stockQuotes.value = data.map(item => ({
      ...item,
      justUpdated: false
    }))
...
const updateStockQuote = (symbol: string, data: any) => {
  const index = stockQuotes.value.findIndex(q => q.symbol === symbol)
  if (index !== -1) {
    // 标记为刚更新
    stockQuotes.value[index] = {
      ...data,
      justUpdated: true
    }
    lastUpdateTime.value = new Date().toLocaleTimeString()
    ...
    // 500ms后移除更新标记
    setTimeout(() => {
      if (stockQuotes.value[index]) {
        stockQuotes.value[index].justUpdated = false
      }
    }, 500)
  } else {
    // 新增股票
    stockQuotes.value.push({
      ...data,
      justUpdated: true
    })

    setTimeout(() => {
      const idx = stockQuotes.value.findIndex(q => q.symbol === symbol)
      if (idx !== -1) {
        stockQuotes.value[idx].justUpdated = false
      }
    }, 500)
  }
}
...
const handleLayoutChange = () => {
  ...
  // 重新订阅所有股票
  if (wsConnected.value) {
    subscribe(selectedStocks.value.slice(0, layoutMode.value))
  }
}

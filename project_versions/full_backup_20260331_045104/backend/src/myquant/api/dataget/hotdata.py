...
@router.get("/debug/memory", summary="内存统计")
async def debug_hotdb_memory():
    """返回 HotDB 内存缓存统计信息"""
    from myquant.core.market.services.hotdb_service import get_hotdb_service
    ...
    if not hotdb or not hasattr(hotdb, 'get_memory_stats'):
        return {"error": "HotDB adapter not available"}

    return hotdb.get_memory_stats()

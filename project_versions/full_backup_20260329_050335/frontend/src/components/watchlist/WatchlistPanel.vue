...
const handleRemoveStock = async (symbol: string) => {
  ...
  if (currentGroup) {
    ...
    try {
      // 清理该股票的所有周期缓存
      const periods = ['1d', '1w', '1mon', '1m', '5m', '15m', '30m', '1h']
      let clearedCount = 0

      periods.forEach(period => {
        const cacheKey = `kline_${symbol}_${period}`
        const cached = localStorage.getItem('myquant_cache')
        if (cached) {
          const cache = JSON.parse(cached)
          if (Array.isArray(cache)) {
            const newCache = cache.filter(([key]: [string]) => key !== cacheKey)
      ...
      if (clearedCount > 0) {
        console.log(`[handleRemoveStock] 已清理 ${symbol} 的 ${clearedCount} 个前端缓存项`)
      }
    } catch (error) {

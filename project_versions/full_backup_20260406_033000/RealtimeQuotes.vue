        ...
        <div class="chart-area">
          <div class="chart-toolbar">
            ...
            <div class="toolbar-right">
              ...
              <!-- MACD指标按钮 -->
              <button
                :class="['timeframe-btn', { active: activeIndicators.includes('MACD') }]"
...
const activeIndicators = ref<string[]>(['MACD'])  // 默认显示MACD
  ...
  try {
    ...
    if (klineRes.data && klineRes.data.length > 0) {
      ...
      if (activeIndicators.value.includes('MACD')) {
        ...
        const macdPane = chart?.panes()[MACD_PANE_INDEX]
        if (macdPane) {
          macdPane.setStretchFactor(savedMacdStretchFactor)
        }
      } else {
        // MACD未被选中，保存伸展因子并隐藏
        const macdPane = chart?.panes()[MACD_PANE_INDEX]
        if (macdPane && indicatorSeries.macd) {
          // 保存伸展因子
          savedMacdStretchFactor = 1  // 默认值，后续可从pane获取
          // 清除数据并隐藏

...
class V5XtQuantAdapter(V5DataAdapter):
    ...
    def _fetch_online_data(
        ...
        """第二层：在线获取（支持日期范围和 count）

        改进：如果提供了日期范围，使用日期范围获取数据
        """
        try:
            start = time.time()
                ...
                if data and xt_symbol in data:
                    ...
                    if df is not None and not df.empty:
                        logger.info(
                            ...
                            f"{len(df)}条(日期范围: {start_date}~{end_date}), 耗时:{elapsed:.1f}ms"
                        )
                        return self._normalize_kline_df(df, 'xtquant_online')

                logger.info(f"[在线获取] {symbol} {xt_period} 日期范围无数据，尝试 count 方式")
            # 如果没有日期范围或日期范围无数据，使用 count 方式
            data = xtdata.get_market_data_ex(
                field_list=field_list,
                stock_list=[xt_symbol],
                period=xt_period,
    ...
    def _sync_download_data(self, symbol: str, xt_period: str) -> bool:
        ...
        try:
            ...
            # 根据周期决定下载范围
            if xt_period == '1d':
                start_date = end_date - timedelta(days=730)  # 2年日线
            elif xt_period == '1w':
                start_date = end_date - timedelta(days=1095)  # 3年周K (~150周)
            elif xt_period == '1mon':
                start_date = end_date - timedelta(days=1825)  # 5年月K (~60月)
            else:
                start_date = end_date - timedelta(days=180)  # 其他周期默认6个月

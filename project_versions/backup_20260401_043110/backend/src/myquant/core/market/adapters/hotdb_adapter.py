...
from ..services.cache_manager_service import get_cache_manager, CachePartition
...
class V5HotDBAdapter(V5DataAdapter):
    ...
    def __init__(self):
        ...
        logger.info(
            ...
            f"统一缓存已启用（RAW_KLINE分区，TTL=300秒）"
        )
    ...
    def get_kline(
        ...
        Args:
            ...
            count: 返回数量
            adjust_type: 复权类型
            bypass_cache: 绕过缓存（用于写操作，确保读取最新数据）
        ...
        def is_trading_hours():
            ...
            # 交易时间：9:15-15:05（涵盖盘前到收盘）
            is_trading = (9 <= current_hour < 15) or (current_hour == 15 and current_minute <= 5)
            weekday = now.weekday()
            is_weekday = weekday < 5  # 0-4 是周一到周五

            return is_trading and is_weekday

        use_cache = not bypass_cache and is_trading_hours()
        ...
        for symbol in symbols:
            try:
                ...
                # L1: 统一缓存检查（仅在交易时间内且未绕过缓存时）
                if use_cache:
                    cached_df = self._cache_manager.get(CachePartition.RAW_KLINE, cache_key)
                    if cached_df is not None:
                        logger.debug(f"[HotDB-L1] 缓存命中: {symbol} {period}")
                ...
                if df is None or df.empty:
                    ...
                    if is_invalid:
                        ...
                        # 清除无效的缓存数据
                        self._cache_manager.delete(CachePartition.RAW_KLINE, cache_key)
                        # 返回 None 让 fallback 继续尝试
                        continue

                # 保存到统一缓存（完整的未过滤数据，1分钟TTL）
                # 缓存策略：只在交易时间内使用，TTL缩短为1分钟减少脏数据窗口
                # 非交易时间不写入缓存，减少内存占用
                if use_cache:
                    self._cache_manager.set(
                        CachePartition.RAW_KLINE, cache_key, df.copy(), ttl=60
                    )
    ...
    def _read_existing_kline(self, symbol: str, period: str) -> Optional[pd.DataFrame]:
        """读取现有的 K线数据（用于合并，直接从文件读取，避开 L1 缓存）"""
        try:
            # 直接从文件读取，不使用 get_kline（避免读取到旧的缓存数据）
            stock_dir = self._get_stock_dir(symbol, period)
            ...
            logger.debug(f"[HotDB] _read_existing_kline 直接从文件读取: {len(df)} 条")
            return df

        except Exception as e:
            logger.warning(f"[HotDB] _read_existing_kline 失败: {e}")
            return None
        ...
        try:
            ...
            if period:
                ...
                if exchange_dir.exists():
                    ...
                    # 清除所有周期的统一缓存
                    # 注意：CacheManagerService 不支持按前缀删除，这里只记录日志
                    logger.info(f"[HotDB] 已删除 {symbol} 所有周期文件，缓存将自动过期")
                    return True
    ...
    def detect_gap(self, symbol: str, period: str) -> Optional[Dict]:
        ...
        try:
            ...
            if period == '1d':
                ...
                df_dict = self.get_kline(symbols=[symbol], period=period, count=10000)

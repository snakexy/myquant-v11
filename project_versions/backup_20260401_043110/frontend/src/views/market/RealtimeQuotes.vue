...
import { dataUpdateProgress } from '@/services/dataUpdateProgress'
...
const checkPriceAlerts = () => {
  ...
  const currentQuote = dataStore.getQuote(selectedStock.value)
...
const loadKlineData = async () => {
  ...
  // 显示数据更新进度
  dataUpdateProgress.startUpdate(
    selectedStock.value,
    currentTimeframe.value,
    `正在加载 ${selectedStock.value} ${currentTimeframe.value} K线数据...`
  )

  try {
    // 第一步：优先只加载K线数据（最核心的）
    dataUpdateProgress.updateProgress(30, '正在从服务器获取数据...')
    ...
    if (klineRes.data && klineRes.data.length > 0) {
      ...
      // 通知进度更新完成
      dataUpdateProgress.setStatus('completed', `${selectedStock.value} ${currentTimeframe.value} 数据加载完成`)

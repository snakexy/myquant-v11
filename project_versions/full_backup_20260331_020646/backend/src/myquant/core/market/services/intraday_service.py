...
class IntradayKlineService:
    ...
    注意：此服务不缓存数据，缓存由 SeamlessKlineService 统一管理
    """

    # 缓存 TTL 配置（秒）- 已废弃，缓存由 SeamlessKlineService 管理
    CACHE_TTL = {
        '1m': 10,     # 1分钟数据缓存10秒
        '5m': 60,     # 5分钟数据缓存60秒
        '15m': 120,   # 15分钟数据缓存2分钟
    ...
    def __init__(self):
        # 不再使用缓存，缓存由上层 SeamlessKlineService 统一管理
    ...
    def get_kline(
        ...
        注意：此服务不缓存数据，use_cache 参数已废弃
        缓存由上层 SeamlessKlineService 统一管理

...
async function loadStockStatus() {
  try {
    const response = await axios.get(`/api/hotdb/status/${props.stockInfo.symbol}`)
    stockStatus.value = {
      ...response.data,
...
async function handleCheckGaps() {
  ...
  try {
    const response = await axios.post('/api/hotdb/check_gaps', {
...
async function handleSmartUpdate() {
  ...
  try {
    const response = await axios.post('/api/hotdb/smart_update', {
      symbol: props.stockInfo.symbol,
...
async function handleDownload() {
  ...
  try {
    ...
    const response = await axios.post('/api/hotdb/download', {
      symbol: props.stockInfo.symbol,
    ...
    downloadResult.value = {
      summary: response.data.summary,
      successCount: Object.values(response.data.results).filter((r: any) => r.status === 'success').length
    }
...
async function handleClear() {
  ...
  try {
    const response = await axios.delete('/api/hotdb/clear', {
      data: {
        symbol: props.stockInfo.symbol,

  ...
  <div class="panel watchlist-panel">
    ...
    <div class="stock-list">
      ...
      >
        ...
        <div class="menu-item warning" @click.stop="handleClearCache">
          <span>{{ isZh ? '删除缓存' : 'Clear Cache' }}</span>
        </div>
...
import StockDetailDialog from '../data-management/StockDetailDialog.vue'
...
function showStockDetail(symbol: string) {
...
function handleDataUpdated() {
  ...
  if (selectedStock.value) {
    selectStock(selectedStock.value)
  }
}
...
const handleRefreshStock = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.success) {
        ElMessage.success(`${symbol} 数据已更新到 HotDB，请点击股票刷新显示`)
      } else {
...
const handleClearCache = async () => {
  ...
  try {
    const { ElMessage } = await import('element-plus')
    ElMessage.info(`${symbol} 正在清除缓存...`)

    // 1. 删除 HotDB 中的数据
    try {
      const result = await deleteHotDBSymbol(symbol)
      if (result.success) {
        console.log(`[handleClearCache] 已删除 HotDB 中的 ${symbol} 数据`)
      } else {
        console.warn(`[handleClearCache] 删除 HotDB 失败: ${result.message}`)
      }
    } catch (error) {
      console.error(`[handleClearCache] 删除 HotDB 出错:`, error)
    }

    // 2. 清理前端缓存
    try {
      const { clearCacheByPrefix } = await import('@/utils/cacheManager')
      clearCacheByPrefix(`kline_${symbol}_`)
      console.log(`[handleClearCache] 已清理 ${symbol} 的 K 线缓存`)
    } catch (error) {
      console.error(`[handleClearCache] 清理前端缓存出错:`, error)
    }

    ElMessage.success(`${symbol} 缓存已清除，正在重新加载...`)
    ...
    setTimeout(async () => {
      try {
        ...
        if (response.ok) {
          const data = await response.json()
          if (data.success) {
            ElMessage.success(`${symbol} 数据已重新加载，请点击股票刷新显示`)
          } else {
        ...
        ElMessage.warning(`缓存已清除，请点击股票重新加载`)
      }
    }, 500)
...
// 删除股票 - 从当前激活分组删除，同时清理 HotDB 数据和前端缓存
const handleRemoveStock = async (symbol: string) => {
  ...
  if (currentGroup) {
    ...
    // ⭐ 清理前端缓存（关键修复）
    try {
      // 使用 cacheManager 清除该股票的所有周期缓存
      const { clearCacheByPrefix } = await import('@/utils/cacheManager')
      clearCacheByPrefix(`kline_${symbol}_`)
      console.log(`[handleRemoveStock] 已清理 ${symbol} 的 K 线缓存`)
    } catch (error) {
      console.error(`[handleRemoveStock] 清理前端缓存出错:`, error)
    }
  }
}

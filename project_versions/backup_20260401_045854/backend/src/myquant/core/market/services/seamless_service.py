...
class SeamlessKlineService:
    ...
    def get_kline(
        ...
        history_key = f"kline:history:{symbol}:{period}:{adjust_type}"
        realtime_key = f"kline:realtime:{symbol}:{period}:{adjust_type}"
        ...
        is_trading = self._is_trading_time() if use_cache else False

        logger.info(f"[缓存检查] {symbol} {period}: use_cache={use_cache}, is_trading={is_trading}")

        if use_cache:
            if is_trading:
                # 盘中：尝试合并历史+实时缓存
                logger.info(f"[缓存-盘中] 尝试读取 history_key, realtime_key")
                history_entry = self._cache_manager.get(CachePartition.MERGED_KLINE, history_key)
                realtime_entry = self._cache_manager.get(CachePartition.REALTIME_KLINE, realtime_key)
                logger.info(f"[缓存-盘中] history_entry={history_entry is not None}, realtime_entry={realtime_entry is not None}")
                ...
                # 解析历史缓存
                if history_entry is not None and isinstance(history_entry, dict) and 'data' in history_entry:
                    history_list = history_entry.get('data')
                    if history_list:
                        history_df = pd.DataFrame(history_list)
                        cached_last_time = history_entry.get('last_time')
                        cached_hash = history_entry.get('hash', '')

                # 解析实时缓存
                if realtime_entry is not None and isinstance(realtime_entry, dict) and 'data' in realtime_entry:
                    realtime_list = realtime_entry.get('data')
                    if realtime_list:
                        realtime_df = pd.DataFrame(realtime_list)
                        if cached_last_time is None:
                            cached_last_time = realtime_entry.get('last_time')
                        if not cached_hash:
                            cached_hash = realtime_entry.get('hash', '')

                # 合并历史+实时
                if history_df is not None or realtime_df is not None:
                    dfs_to_merge = []
                    if history_df is not None and not history_df.empty:
                        dfs_to_merge.append(history_df)
                    if realtime_df is not None and not realtime_df.empty:
                        dfs_to_merge.append(realtime_df)

                    if dfs_to_merge:
                        cached_data = pd.concat(dfs_to_merge, ignore_index=True)
                        # 去重并排序
                        if 'datetime' in cached_data.columns:
                            cached_data = cached_data.drop_duplicates(subset=['datetime'], keep='last')
                            cached_data = cached_data.sort_values('datetime').reset_index(drop=True)
                            if not cached_data.empty:
                                cached_last_time = cached_data.iloc[-1]['datetime']

                        logger.info(f"[双层缓存-命中] {symbol} {period}: "
                                   f"历史={len(history_df) if history_df is not None else 0}条, "
                                   f"实时={len(realtime_df) if realtime_df is not None else 0}条, "
                                   f"合并={len(cached_data)}条")

                        # Hash-based cache invalidation for 盘中
                        current_hash = self._get_current_fingerprint_hash(symbol, period)
                        if cached_hash and current_hash:
                            if cached_hash == current_hash:
                                ...
                                logger.info(f"[Hash验证] {symbol} {period}: 盘中缓存失效，触发增量获取")
                        else:
                            logger.debug(f"[Hash验证] {symbol} {period}: 无hash信息，继续增量获取")

                        incremental_fetch = True
            else:
                # 盘后：只用历史缓存（直接使用 cache_key）
                logger.info(f"[缓存-盘后] 读取 cache_key")
                cached_entry = self._cache_manager.get(CachePartition.MERGED_KLINE, cache_key)
                logger.info(f"[缓存-盘后] cache_key 命中={cached_entry is not None}")
                if cached_entry is not None:
                    # 新缓存条目格式: {'data': dict_list, 'last_time': timestamp, 'hash': hash_value}
                    ...
                    if cached_data is not None and not cached_data.empty:
                        logger.info(f"[单层缓存-命中] {symbol} {period} {adjust_type}, "
                        ...
                        if cached_hash and current_hash:
                            if cached_hash == current_hash:
                                ...
                                logger.info(f"[Hash验证] {symbol} {period}: 缓存失效 (cached={cached_hash[:8]}..., current={current_hash[:8]}...)，触发增量获取")
                        else:
                            logger.debug(f"[Hash验证] {symbol} {period}: 无hash信息，继续增量获取")
        ...
        if incremental_fetch and cached_last_time is not None:
            ...
            logger.debug(f"[缓存直接返回] {symbol} {period}")
            return self._ensure_naive_datetime(cached_data.copy())
        ...
        if period == '1d':
            ...
            if missing_dates:
                logger.info(f"[自动补全] 发现 {len(missing_dates)} 个缺口，触发后台智能补全")
                # 在后台触发智能补全（不阻塞当前请求）
                try:
                    ...
                    # 异步触发：不等待结果，让补全在后台进行
                    import threading
                    def background_smart_update():
                        try:
                            hotdb_service.smart_update(symbol, period)
                            logger.info(f"[自动补全] {symbol} {period} 后台补全完成")
                        except Exception as e:
                            logger.warning(f"[自动补全] {symbol} {period} 后台补全失败: {e}")
                    thread = threading.Thread(target=background_smart_update, daemon=True)
                    thread.start()
                except Exception as e:
                    logger.warning(f"[自动补全] 触发后台补全失败: {e}")
        ...
        if use_cache and not historical.df.empty:
            ...
            df_len = len(historical.df)

            logger.info(f"[缓存写入] {symbol} {period}: is_trading={is_trading}, df_len={df_len}, use_cache={use_cache}")

            # 获取当前数据指纹hash
            data_hash = self._get_current_fingerprint_hash(symbol, period) or self._calculate_data_hash(historical.df)
            logger.debug(f"[缓存写入] {symbol} {period}: hash={data_hash[:8] if data_hash else 'N/A'}...")

            # 缓存键设计
            history_key = f"kline:history:{symbol}:{period}:{adjust_type}"
            realtime_key = f"kline:realtime:{symbol}:{period}:{adjust_type}"

            if is_trading and df_len > 100:
                # 盘中：双层缓存分离
                # 实时缓存：最新100条，10秒TTL
                realtime_df = historical.df.tail(100).copy()
                realtime_entry = {
                    'data': realtime_df.to_dict('records'),
                    'last_time': realtime_df.iloc[-1]['datetime'],
                    'count': len(realtime_df),
                    'hash': data_hash
                }
                self._cache_manager.set(CachePartition.REALTIME_KLINE, realtime_key, realtime_entry, ttl=10)
                logger.info(f"[双层缓存-实时] {symbol} {period}: 缓存100条 (TTL=10s)")

                # 历史缓存：除最新100条外的数据，1小时TTL
                history_df = historical.df.iloc[:-100].copy()
                if not history_df.empty:
                    history_entry = {
                        'data': history_df.to_dict('records'),
                        'last_time': history_df.iloc[-1]['datetime'],
                        'count': len(history_df),
                        'hash': data_hash
                    }
                    self._cache_manager.set(CachePartition.MERGED_KLINE, history_key, history_entry, ttl=3600)
                    logger.info(f"[双层缓存-历史] {symbol} {period}: 缓存{len(history_df)}条 (TTL=1h)")
            else:
                # 盘后或数据量少：单层缓存（完整数据）
                cache_df = historical.df.tail(CACHE_SIZE).copy() if df_len > CACHE_SIZE else historical.df.copy()
                cache_entry = {
                ...
                ttl = 3600 if period == '1d' else 600  # 日线1小时，分钟线10分钟
                self._cache_manager.set(CachePartition.MERGED_KLINE, cache_key, cache_entry, ttl=ttl)
                logger.info(f"[单层缓存] {symbol} {period}: 缓存{len(cache_df)}条 (TTL={ttl}s)")
        ...
        if count:
            # 周K/月K返回全部数据（数据量不大），其他周期按用户请求的数量返回
            if period in ('1w', '1W', 'week', '1mon', '1M', 'month'):
                logger.info(f"[数量限制] {symbol} {period}: 周K/月K返回全部数据，跳过 count={count} 限制")
            else:
                return_count = count
    ...
    def _check_and_trigger_update(self, symbol: str, period: str) -> None:
        """检查数据新鲜度并在后台触发自动补全
        ...
        try:
            ...
            # 【智能策略】避免频繁重复更新（同一股票同一周期5分钟内不重复触发）
            update_key = f"{symbol}:{period}"
            last_update_time = self._last_update_times.get(update_key)
            if last_update_time is not None:
                elapsed = (now - last_update_time).total_seconds()
                if elapsed < 300:  # 5分钟内不重复触发
                    logger.debug(f"[自动补全] {symbol} {period} 距上次更新仅 {elapsed:.0f} 秒，跳过")
                    return
            ...
            needs_update = False
            reason = ""
            update_mode = "smart"  # smart=智能更新（只更新最后一根）, full=完整补全

            latest = gap_info.get('latest')

            # 判断是否有数据（检查 latest 是否为 None）
            has_data = latest is not None

            if gap_info.get('has_gap'):
                gap_reason = gap_info.get('reason', '')

                # 【智能判断时机】对于小缺口（如几分钟），延迟批量处理
                if gap_reason == 'intraday_gap':
                    minutes_missing = gap_info.get('minutes_missing', 0)
                    if minutes_missing < 30:  # 缺口小于30分钟，不紧急处理
                        logger.debug(f"[自动补全] {symbol} {period} 缺口仅 {minutes_missing} 分钟，延迟处理")
                        return
                ...
                # 有数据但可能过期 - 检查周线/月线是否需要更新最后一根
                if isinstance(latest, str):
                    latest = pd.to_datetime(latest)

                now = pd.Timestamp.now(tz=None).replace(tzinfo=None)

                # 周线、月线：智能更新（只更新最后一根K线，确保当天的数据是最新的）
                if period in ['1w', '1W', '1M', '1mon']:
                    # 检查最后一根K线的日期是否是本周/本月
                    if period in ['1w', '1W']:
                        # 周线：检查最后数据是否在当前周
                        week_start = now - pd.Timedelta(days=now.weekday())
                        week_start = week_start.replace(hour=0, minute=0, second=0, microsecond=0)
                        needs_update = latest < week_start
                    else:  # 1M, 1mon
                        # 月线：检查最后数据是否在当前月
                        month_start = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
                        needs_update = latest < month_start

                    if needs_update:
                        reason = f'update_last_bar_{period}'
                        update_mode = "smart"

            if needs_update:
                logger.info(f"[自动补全] {symbol} {period}: 检测到需要补全 ({reason})，触发后台更新")

                # 记录更新时间（避免5分钟内重复触发）
                update_key = f"{symbol}:{period}"
                self._last_update_times[update_key] = pd.Timestamp.now(tz=None).replace(tzinfo=None)

                # 在后台触发智能补全（不阻塞当前请求）
                import threading
                def background_smart_update():
                    try:
                        # 对于周线/月线且无缺口，从在线源获取最后几根数据更新
                        if update_mode == "smart" and period in ['1w', '1W', '1M', '1mon']:
                            # 从在线源获取最后5根数据（会覆盖旧的最后一根）
                            from myquant.core.market.adapters import get_adapter
                            # 周线/月线使用 PyTdx（避免结算日期冲突）
                            pytdx = get_adapter('pytdx')
                            if pytdx and pytdx.is_available():
                                df_dict = pytdx.get_kline(
                                    symbols=[symbol],
                                    period=period,
                                    count=5
                                )
                                if symbol in df_dict and not df_dict[symbol].empty:
                                    # 保存到 HotDB（会根据 datetime 去重，覆盖旧数据）
                                    hotdb_service.get_adapter('hotdb').save_kline(
                                        symbol, df_dict[symbol], period
                                    )
                                    logger.info(f"[自动补全] {symbol} {period} 从 PyTdx 更新最后一根K线完成")
                        else:
                            # 完整补全
                            hotdb_service.smart_update(symbol, period)
                            logger.info(f"[自动补全] {symbol} {period} 后台补全完成")
            ...
            logger.warning(f"[自动补全] {symbol} {period} 检查失败: {e}")
    ...
    def _is_trading_time(self) -> bool:
        ...
        try:
            ...
            return detector.is_trading_time()
        except Exception as e:
            logger.debug(f"[交易时间检测] 判断失败: {e}，默认为非交易时间")
            return False
    ...
    def _calculate_data_hash(self, df: pd.DataFrame) -> str:
        ...
        # 使用 count + latest_dt 作为简单的 hash key
        # 与 DailyDataStatusService 的指纹格式保持一致
        try:
            latest_dt = df.iloc[-1]['datetime']
            if isinstance(latest_dt, pd.Timestamp):
                latest_dt = latest_dt.strftime('%Y-%m-%d %H:%M:%S')
            count = len(df)
            return f"{count}_{latest_dt}"
        except Exception as e:
            logger.debug(f"[Hash计算] 失败: {e}")
            return ""

    def _get_current_fingerprint_hash(self, symbol: str, period: str) -> Optional[str]:
        """获取当前数据指纹哈希

        从 DailyDataStatusService 获取最新的数据指纹
        ...
        """
        try:
            from .daily_data_status import get_daily_status_service
            status_service = get_daily_status_service()
            fingerprint = status_service.get_fingerprint(symbol, period)
            if fingerprint:
                return fingerprint.hash
            return None
        except Exception as e:
            logger.debug(f"[指纹获取] 失败: {e}")
            return None

...
class HotDBService:
    ...
    def __init__(self):
        ...
        # === 双层缓存：历史缓存 + 实时缓存 ===
        # 历史缓存：容量100只股票，TTL 1小时
        self._historical_cache = TTLCache(maxsize=100, ttl=3600)
        # 实时缓存：容量动态=自选股总数，TTL 10秒（盘中）
        self._realtime_cache = TTLCache(maxsize=50, ttl=10)

        # 记录自选股数量，用于调整实时缓存容量
        self._watchlist_count = 50
    ...
    def _update_watchlist_cache_size(self) -> None:
        """更新自选股数量并调整实时缓存容量"""
        try:
            # 读取自选股数据
            watchlist_file = Path(__file__).parent.parent.parent.parent.parent / "data" / "watchlist.json"
            ...
            new_count = len(unique_symbols)
            if new_count != self._watchlist_count:
                self._watchlist_count = new_count
                # 重建实时缓存，使用新容量
                from cachetools import TTLCache
                old_cache = self._realtime_cache
                self._realtime_cache = TTLCache(maxsize=new_count, ttl=10)
                logger.info(f"[HotDBService] 更新实时缓存容量: {new_count}只股票")
        except Exception as e:
            logger.debug(f"[HotDBService] 更新自选股数量失败: {e}")

    def _compute_fingerprint(self, df: pd.DataFrame) -> str:
        """计算数据指纹

        使用轻量级哈希算法，只计算最近50条数据的关键字段
        ...
        if not pd.api.types.is_datetime64_any_dtype(recent_df['datetime']):
            recent_df = recent_df.copy()
            recent_df['datetime'] = pd.to_datetime(recent_df['datetime'])

        # 组合关键字段进行哈希
        # 使用 datetime 和 close 价格的组合（体积小但能反映数据变化）
        key_data = (
            recent_df['datetime'].dt.strftime('%Y%m%d%H%M%S').tolist() +
            recent_df['close'].round(6).tolist()
        )

        # 转换为字符串并计算哈希
        key_str = json.dumps(key_data, sort_keys=True)
        return hashlib.md5(key_str.encode()).hexdigest()[:16]  # 取前16位
            ...
            for period in periods:
                try:
                    ...
                    if symbol in df_dict and not df_dict[symbol].empty:
                        ...
                        if success:
                            # 更新数据指纹 - 使用保存后的完整数据
                            updated_df_dict = hotdb.get_kline(symbols=[symbol], period=period, count=100)
                            if symbol in updated_df_dict and not updated_df_dict[symbol].empty:
                                self._update_fingerprint(symbol, period, updated_df_dict[symbol])
    ...
    def _aggregate_from_5m(self, symbol: str, target_period: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                # 更新数据指纹 - 使用保存后的完整数据
                updated_df_dict = hotdb.get_kline(symbols=[symbol], period=target_period, count=100)
                if symbol in updated_df_dict and not updated_df_dict[symbol].empty:
                    self._update_fingerprint(symbol, target_period, updated_df_dict[symbol])
    ...
    def _detect_gap(self, symbol: str, period: str) -> Optional[Dict]:
        ...
        """
        try:
            # === 短期缓存：5分钟内同一股票+周期跳过检测 ===
            cache_key = f"{symbol}_{period}"
            current_time = time.time()
            if cache_key in self._gap_check_cache:
                last_check_time = self._gap_check_cache[cache_key]
                if current_time - last_check_time < 300:  # 5分钟 = 300秒
                    logger.debug(f"[HotDBService] {symbol} {period} 使用短期缓存，跳过缺口检测")
                    return {'has_gap': False, 'reason': 'cached', 'latest': None}
            # 更新缓存时间
            self._gap_check_cache[cache_key] = current_time
            ...
            if period == '1d':
                ...
                # 0. 【首先】检查当前是否在交易时间内
                # 如果不在交易时间内（凌晨、午休、周末、节假日），不检查"数据太旧"
                current_dt = datetime.now()
                is_in_trading_time = detector.is_trading_time(current_dt)

                if not is_in_trading_time:
                    # 不在交易时间内，只检查内部缺口，不检查"数据太旧"
                    logger.debug(f"[HotDBService] {symbol} {period} 当前不在交易时间内，跳过最新数据缺口检测")
                else:
                    # 1. 【优先】检查最新数据与当前时间的差距
                    # 这是最重要的检查：数据是否太旧了？
                ...
                # 2. 检查内部缺口（使用哈希校验）
                # 先检查数据指纹，如果数据未变化则跳过检查
                from .daily_data_status import get_daily_status_service
                status_service = get_daily_status_service()

                # 获取当前数据的指纹
                df_dict_quick = hotdb.get_kline(
                    symbols=[symbol],
                    period=period,
                    count=100  # 只获取最近100条用于计算指纹
                )

                current_fingerprint = None
                if symbol in df_dict_quick and not df_dict_quick[symbol].empty:
                    current_df = df_dict_quick[symbol]
                    current_count = len(current_df)
                    current_latest = current_df['datetime'].iloc[-1].strftime('%Y-%m-%d %H:%M:%S')
                    current_hash = self._compute_fingerprint(current_df)
                    current_fingerprint = (current_count, current_latest, current_hash)

                    # 验证指纹
                    if status_service.is_fingerprint_valid(
                        symbol, period,
                        current_count, current_latest, current_hash
                    ):
                        # 指纹有效，数据未变化，跳过内部缺口检测
                        logger.debug(f"[HotDBService] {symbol} {period} 指纹有效，跳过内部缺口检测")
                    else:
                        # 指纹失效或无指纹，执行完整内部缺口检测
                        logger.info(f"[HotDBService] {symbol} {period} 指纹失效，执行内部缺口检测")
                        df_dict_full = hotdb.get_kline(
                            symbols=[symbol],
                            period=period,
                            count=10000
                        )

                        if symbol in df_dict_full and not df_dict_full[symbol].empty:
                            df = df_dict_full[symbol].copy()
                            df = df.sort_values('datetime').reset_index(drop=True)
    ...
    def _update_fingerprint(self, symbol: str, period: str, df: pd.DataFrame) -> None:
        """更新数据指纹

        在数据补全或更新后调用，更新状态文件中的指纹记录
        ...
        try:
            ...
            count = len(df)
            # 确保 datetime 列是 datetime 类型
            latest_dt_value = df['datetime'].iloc[-1]
            if isinstance(latest_dt_value, str):
                latest_dt = latest_dt_value
            else:
                latest_dt = latest_dt_value.strftime('%Y-%m-%d %H:%M:%S')
            hash_value = self._compute_fingerprint(df)

            status_service.save_fingerprint(symbol, period, count, latest_dt, hash_value)

            logger.debug(f"[HotDBService] 更新指纹: {symbol} {period}, count={count}")
        except Exception as e:
            logger.warning(f"[HotDBService] 更新指纹失败: {e}")
        ...
        try:
            ...
            try:
                ...
                if symbol in result and not result[symbol].empty:
                    ...
                    # 更新数据指纹 - 使用保存后的完整数据
                    updated_df_dict = hotdb.get_kline(symbols=[symbol], period='1d', count=100)
                    if symbol in updated_df_dict and not updated_df_dict[symbol].empty:
                        self._update_fingerprint(symbol, '1d', updated_df_dict[symbol])
            ...
            try:
                ...
                if symbol in result and not result[symbol].empty:
                    ...
                    # 更新数据指纹 - 使用保存后的完整数据
                    updated_df_dict = hotdb.get_kline(symbols=[symbol], period='5m', count=100)
                    if symbol in updated_df_dict and not updated_df_dict[symbol].empty:
                        self._update_fingerprint(symbol, '5m', updated_df_dict[symbol])

      ...
      <div class="panel-content">
        ...
        <div class="section">
          ...
          <div class="groups-list">
            ...
            <!-- 空状态 -->
            <div v-if="dataStore.watchlistGroups.length === 0" class="empty-state">
              {{ isZh ? '暂无分组' : 'No groups' }}
            </div>
          </div>
        </div>
...
const updateGroupInterval = (groupId: string, event: Event) => {
  ...
  addLog('info', `分组 ${dataStore.watchlistGroups.find(g => g.id === groupId)?.name} 刷新间隔设置为 ${interval}ms`)
}
...
const refreshStats = () => {
  // 模拟统计数据（实际应从调度器获取）
  stats.value = {
    activeTasks: dataStore.watchlistGroups.filter(g => g.stocks.length > 0).length,

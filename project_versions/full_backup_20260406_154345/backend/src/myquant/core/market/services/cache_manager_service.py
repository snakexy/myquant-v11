...
class CachePartition(Enum):
    ...
    REALTIME_KLINE = "realtime_kline"  # 实时K线（最新增量，盘中专用）
    XDXR = "xdxr"                 # 除权除息数据
    FACTOR = "factor"             # 复权因子
...
DEFAULT_PARTITION_CONFIGS: Dict[CachePartition, PartitionConfig] = {
    ...
    # 历史K线分区：启用（存储完整历史数据，1小时TTL）
    CachePartition.MERGED_KLINE: PartitionConfig(
        maxsize=200,  # 200个股票+周期组合
        default_ttl=3600,  # 1小时
        max_memory_mb=50,  # 最多50MB
        description="历史K线数据"
    ),

    # 实时K线分区：启用（存储最新增量，10秒TTL）
    CachePartition.REALTIME_KLINE: PartitionConfig(
        maxsize=100,  # 100个股票+周期组合
        default_ttl=10,  # 10秒（盘中高频刷新）
        max_memory_mb=10,  # 最多10MB
        description="实时K线增量"
    ),

    # XDXR分区：启用（复权因子数据，7天TTL）
    CachePartition.XDXR: PartitionConfig(
        maxsize=500,
    ...
    # 因子分区：启用（复权因子表，7天TTL）
    CachePartition.FACTOR: PartitionConfig(
        maxsize=500,
        default_ttl=604800,  # 7天
        max_memory_mb=10,
        description="复权因子表"

...
class SimulatorExecutor:
    ...
    def _retrieve_orders_from_decision(self, trade_decision: BaseTradeDecision) -> List[Order]:
        """
        从交易决策中提取订单
    ...
    def _collect_data(
        ...
        if self.verbose:
            print(f"[I {trade_start_time:%Y-%m-%d %H:%M:%S}]: 开始执行交易决策")
        ...
        for order in orders:
            try:
                ...
                if self.verbose:
                    print(
                        f"[I {trade_start_time:%Y-%m-%d %H:%M:%S}]: "
                        f"{'buy' if order.direction > 0 else 'sell'} "
                        f"{order.stock_id}, price {trade_price:.2f}, amount {order.amount}, "
                        f"deal_amount {order.deal_amount}, factor {order.factor}, "
                        f"value {trade_val:.2f}, cash {self.trade_account.get_cash():.2f}."
                    )
    ...
    def execute(self, trade_decision: BaseTradeDecision, level: int = 0) -> List[object]:
        ...
        # 收集数据
        for _ in self.collect_data(trade_decision, level=level):
            pass

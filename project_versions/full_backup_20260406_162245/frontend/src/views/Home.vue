...
// 功能卡片数据 - 根据三阶段工作流程设计大纲修改为4个大卡片
const functionCards = ref([
  {
    id: 1,
    title: '监控',
    description: '监控系统入口 - 多股票同列监控、多阶段走势显示、预测预警功能、市场阶段判断、热门股识别与预测',
    icon: '📊',
    route: '/monitoring',
  ...
  },
  {
    id: 2,
    title: '研究',
    description: '研究阶段入口 - 数据探索、策略构思、模型训练、初步分析、AI助手策略构思',
    icon: '🔬',
    route: '/research',
  ...
  },
  {
    id: 3,
    title: '验证',
    description: '验证阶段入口 - 历史回测、模拟实盘、实时监控、风险评估、参数优化、性能分析',
    icon: '✅',
    route: '/validation',
  ...
  },
  {
    id: 4,
    title: '上线',
    description: '上线阶段入口 - 实盘交易、实时监控、风险控制、仓位管理、绩效跟踪',
    icon: '🚀',
    route: '/production',
...
.monitoring-theme:hover .icon-container {
  filter:
    drop-shadow(0 0 12px rgba(255, 102, 0, 0.8))
    drop-shadow(0 0 24px rgba(255, 102, 0, 0.6));
  transform: scale(1.1);
  animation: iconBounce 0.6s ease-in-out infinite;
}

/* 研究阶段悬停效果 */
.research-theme .icon-container {
  filter: none;
  transition: all 0.3s ease;
}

.research-theme:hover .icon-container {
  filter:
    drop-shadow(0 0 12px rgba(52, 152, 219, 0.8))
...
/* 验证阶段悬停效果 */
.validation-theme .icon-container {
  filter: none;
  transition: all 0.3s ease;
}

.validation-theme:hover .icon-container {
  filter:
    drop-shadow(0 0 12px rgba(156, 39, 176, 0.8))
    drop-shadow(0 0 24px rgba(156, 39, 176, 0.6));
  transform: scale(1.1);
  animation: iconBounce 0.6s ease-in-out infinite;
}

/* 上线阶段悬停效果 */
.production-theme .icon-container {
  filter: none;
  transition: all 0.3s ease;
}

.production-theme:hover .icon-container {
  filter:
    drop-shadow(0 0 12px rgba(76, 175, 80, 0.8))
    drop-shadow(0 0 24px rgba(76, 175, 80, 0.6));
  transform: scale(1.1);
  animation: iconBounce 0.6s ease-in-out infinite;
}
...
@media (max-width: 768px) {
  ...
  .card-icon {
    width: 60px; /* 移动端适当减小图标 */
    height: 60px;
    margin-bottom: 16px;
  }
  
  .icon-container {
    font-size: 60px; /* 移动端图标也相应调整 */
  }
  
  .card-text h2 {
    font-size: 24px; /* 移动端标题字体 */
  }
  
  .card-text p {
    font-size: 16px; /* 移动端描述文字 */
  }
}
</style>
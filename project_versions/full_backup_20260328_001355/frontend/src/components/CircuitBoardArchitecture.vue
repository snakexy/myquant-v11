...
const visibleNodes = computed(() => {
  ...
  if (currentViewMode.value === 'layer') {
    nodes = nodes.sort((a, b) => {
      const layerOrder = ['data_hub', 'qlib_core', 'business_logic', 'ai_strategy', 'live_trading', 'experiment_mgmt', 'application_service']
...
const generateArchitectureConnections = (): ArchitectureConnection[] => {
  ...
  // 生成层级内连接（优化版）
  const layers = ['data_hub', 'qlib_core', 'business_logic', 'ai_strategy', 'live_trading', 'experiment_mgmt', 'application_service']

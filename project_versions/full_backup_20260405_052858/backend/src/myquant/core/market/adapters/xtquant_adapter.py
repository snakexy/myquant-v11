...
class V5XtQuantAdapter(V5DataAdapter):
    ...
    def _normalize_kline_df(self, df: pd.DataFrame, source: str) -> pd.DataFrame:
        ...
        修复：time 列的时区有问题，优先使用 index（日期字符串）构造 datetime
        """
        if df is None or df.empty:
            return df

        # XtQuant 返回的 index 是日期字符串（如 '20260403'），time 列可能有时区问题
        # 优先使用 index 构造正确的 datetime
        if 'datetime' not in df.columns:
            # 检查 index 是否为日期字符串格式（8位数字）
            if isinstance(df.index, pd.Index) and len(df.index) > 0:
                first_idx = str(df.index[0])
                if first_idx.isdigit() and len(first_idx) == 8:
                    # 使用 index 构造 datetime（YYYYMMDD → datetime）
                    df['datetime'] = pd.to_datetime(df.index, format='%Y%m%d')
                    if 'time' in df.columns:
                        df = df.drop(columns=['time'])
                elif 'time' in df.columns:
                    # 降级：使用 time 列（时间戳，毫秒）转换为 datetime

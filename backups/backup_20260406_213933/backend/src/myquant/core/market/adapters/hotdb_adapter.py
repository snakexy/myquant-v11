...
class V5HotDBAdapter(V5DataAdapter):
                    ...
                    if dates and closes:
                        ...
                        if rows:
                            ...
                            if not df.empty:
                                # 检查数据量是否足够
                                if count and len(df) < count:
                                    # 数据不够，尝试从 5m 聚合补充（15m/30m/1h）
                                    if period in ('15m', '30m', '1h'):
                                        need_count = count - len(df)
                                        logger.info(
                                            f"[HotDB] {symbol} {period} 数据不足 "
                                            f"({len(df)}/{count})，尝试从 5m 聚合补充"
                                        )
                                        df_agg = self._aggregate_from_5m(
                                            symbol, period
                                        )
                                        if df_agg is not None and not df_agg.empty:
                                            ...
                                            df_new = df_agg[
                                                ~df_agg['datetime'].isin(existing_times)
                                            ].copy()

                                            if not df_new.empty:
                                                # 合并数据
                                                df = pd.concat(
                                                    [df, df_new], ignore_index=True
                                                )
    ...
    def get_data_info(self, symbol: str, period: str = '1d') -> dict:
        """获取指定股票周期的数据信息
        ...
        try:
            hotdb_period = self._hotdb_period(period)
            # 构建完整路径：data/hotdata/{exchange}/{period}/{dir_name}/
            dir_path = self._data_dir / self._exchange(symbol) / hotdb_period / self._dir_name(symbol)

            # 读取日期和收盘价文件 (文件名格式: {property}.{period}.bin)
            dates_file = dir_path / f"date.{hotdb_period}.bin"
            closes_file = dir_path / f"close.{hotdb_period}.bin"

            if not dates_file.exists() or not closes_file.exists():
                return {'has_data': False, 'count': 0, 'earliest': None, 'latest': None}

            dates = self._read_dates(dates_file)
            if not dates:
                return {'has_data': False, 'count': 0, 'earliest': None, 'latest': None}
            ...
            return {
                ...
                'latest': pd.to_datetime(dates[-1])
            }
        except Exception as e:
            logger.error(f"[HotDB] 获取数据信息失败: {symbol} {period}, {e}")
            return {'has_data': False, 'count': 0, 'earliest': None, 'latest': None}

    def delete_kline(self, symbol: str, period: str = None) -> bool:
        """删除指定股票周期的K线数据
        ...
        """
        try:
            dir_path = Path(self._dir_name(symbol))
            ...
            if period:
                # 删除指定周期的文件
                hotdb_period = self._hotdb_period(period)
                extensions = ['.date', '.open', '.high', '.low', '.close', '.volume', '.amount']
                for ext in extensions:
                    file_path = dir_path / f"{hotdb_period}{ext}"
                    if file_path.exists():
                        file_path.unlink()
                logger.info(f"[HotDB] 已删除 {symbol} {period} 数据")
            else:
                # 删除整个股票目录
                import shutil
                shutil.rmtree(dir_path)
                logger.info(f"[HotDB] 已删除 {symbol} 所有数据")

            return True
        except Exception as e:
            logger.error(f"[HotDB] 删除数据失败: {symbol} {period}, {e}")
            return False

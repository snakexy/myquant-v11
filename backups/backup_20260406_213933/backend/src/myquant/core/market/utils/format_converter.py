...
class FormatConverter:
    ...
    def _xtquant_kline_to_standard(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'datetime' in df_normalized.columns and not df_normalized['datetime'].isna().all():
            ...
            if isinstance(first_val, (pd.Timestamp, datetime)):
                df_normalized['datetime'] = pd.to_datetime(df_normalized['datetime'], errors='coerce')

            # 情况2：是 numeric 毫秒时间戳
            elif isinstance(first_val, (int, float, np.number)) and first_val > 100000000000:
                df_normalized['datetime'] = pd.to_datetime(df_normalized['datetime'], unit='ms', errors='coerce')

        # 时间校正：确保 K线时间在合理范围内
        df_normalized = FormatConverter._correct_kline_time(df_normalized)
    ...
    @staticmethod
    def _correct_kline_time(df: pd.DataFrame) -> pd.DataFrame:
        """
        校正 K线时间，确保在合理的交易时间范围内

        处理策略：
        1. 过滤明显错误的时间（凌晨 0-8 点，晚上 19 点后）
        2. 记录异常时间用于调试
        """
        ...
        def is_valid_trading_time(dt: pd.Timestamp) -> bool:
            """检查是否是有效的交易时间"""
            if pd.isna(dt):
                return False
            ...
            # 下午交易：13:00-15:00
            if hour == 13 or (hour == 14 and minute >= 0) or (hour == 15 and minute == 0):
                return True
        ...
        # 应用时间过滤
        df_filtered = df[df['datetime'].apply(is_valid_trading_time)].copy()

        if len(df_filtered) < original_count:
            logger.warning(
                f"[时间校正] 过滤了 {original_count - len(df_filtered)} 条异常时间数据 "
                f"(原始{original_count}条，保留{len(df_filtered)}条)"
            )
